...
\begin{itemize}
    \item We propose a hierarchical classification framework aligned with practical maintenance workflows, explicitly separating the safety decision (fault detection) from the diagnostic decision (fault typing). This safety-first approach guarantees 100\% fault detection rather than optimizing aggregate metrics.

    \item We demonstrate that entropy features (sample entropy, permutation entropy) achieve \textbf{100\% fault detection rate} across all imbalance ratios (IR = 3, 5, 10), establishing them as the optimal choice for Level 1 safety-critical detection.

    \item We analyze the feature-task alignment principle: entropy features reliably detect fault \emph{presence} but are insufficient for fault \emph{type} classification (33\%, random level). This empirical finding validates the theoretical property of entropy measures and motivates adaptive feature selection across hierarchy levels.

    \item We provide a reproducible experimental framework that simulates industrial imbalance scenarios using bootstrap resampling, with code and data publicly available.
\end{itemize}
...
The 33\% accuracy at Level 2 is not a method failure; it is an empirical confirmation of a fundamental theoretical boundary. Distinguishing between fault types (Ball vs. Inner Race vs. Outer Race) requires understanding the \emph{frequency signatures} of different fault mechanisms:
...
These frequency-domain characteristics are not captured by entropy measures, because entropy quantifies signal \emph{regularity} (not frequency composition). All three fault types share one property: they all produce irregular, unpredictable vibration signals. The entropy measure cannot distinguish between ``irregular signal type A'' and ``irregular signal type B'' because it was never designed to capture fault-mechanism-specific signatures.

This validates the \emph{feature-task alignment} principle: entropy features are optimal for tasks that depend on signal regularity (fault presence detection), but are insufficient for tasks that depend on frequency signatures (fault type classification).
...
\begin{enumerate}
    ...
    \item \textbf{Feature-task alignment is task-specific}: Entropy features achieve 100\% at Level 1 but only 33\% (random level) at Level 2. This empirical finding validates the theoretical property of entropy measures---they capture signal regularity but not fault-type-specific frequency signatures.
